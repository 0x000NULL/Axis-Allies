...
export class WasmEngine {
    ...
    static engineVersion() {
        ...
        let deferred1_1;
        try {
            const ret = wasm.wasmengine_engineVersion();
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
            return getStringFromWasm0(ret[0], ret[1]);
        } finally {
    ...
    static loadFromSave(data) {
        ...
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.wasmengine_loadFromSave(ptr0, len0);
        if (ret[2]) {
            throw takeFromExternrefTable0(ret[1]);
        }
        return WasmEngine.__wrap(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('aa_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
